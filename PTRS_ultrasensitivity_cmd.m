...
%% --------------- Generate final_fig. 2A-2I - Ultrasensitivity in response to k0 --------------------------------- %%
tspan = [0:100:1000000]; 
init = default_init;
param = default_param;
param.k0 = 0.1;
...
%% --------------- Generate final_fig. 3A - Mechanism for ultrasensitivity ---------------------------------------- %%
param = default_param;
...
%% --------------- Generate final_fig. 3B - Mechanism for ultrasensitivity ---------------------------------------- %%
init = default_init;
param = default_param;
param.H2O2_switch = 0; % set to 0 to clamp H2O2  
H2O2_vector = [];
...
%% --------------- Generate final_fig. 3C - Mechanism for ultrasensitivity ---------------------------------------- %%
init = default_init;
param = default_param;
...
%
%% --------------- Generate final_fig. 3D - Mechanism for ultrasensitivity ---------------------------------------- %%
init = default_init;
param = default_param;
param.H2O2_switch = 0; % set to 0 to clamp H2O2  
H2O2_vector = []; 
...
%
%% --------------- Generate final_fig. 3E - Mechanism for ultrasensitivity ---------------------------------------- %%
...
%
%% --------------- Generate final_fig. 3F - Mechanism for ultrasensitivity ---------------------------------------- %%
init = default_init;
param = default_param;
param.H2O2_switch = 0; % set to 0 to clamp H2O2  
H2O2_vector = []; 
...
%% --------------- Generate final_fig. 4A-4G-S1 - Sensitivity analysis ----------------------------------------------- %%
tspan = [0:100:1000000]; 
init = default_init;
param = default_param;
param.k0 = 0.1;
...
%% --------------- Generate final_fig. S2A-S2F - Time course for varying k0 --------------------------------------- %%
...
%% --------------- Generate fig. S6A and B ---------------------------------------------------------------------- %%
tspan = [0:100:1e6]; 
init = default_init;
param = default_param;
param.k0 = 160; % run with different param.k0 values (80, 100, 120, 140, 160);
param.SRXtot = 0.001;
...
while param.SRXtot <= 10
        y0 = cell2mat(struct2cell(init));
        [t,y] = ode15s('PTRS_ode', tspan, y0, [], param);
        SRXtot_vector = [SRXtot_vector, param.SRXtot]; 
        H2O2_vector = [H2O2_vector, y(end,1)];
        PRXSO2H_vector = [PRXSO2H_vector, y(end,5)];
        param.SRXtot = param.SRXtot * increment; 
end          

% H2O2
figure(9018)
loglog(SRXtot_vector, H2O2_vector, 'Color', [51,102,255]/255, 'LineWidth', 3);
hold on  
xlim([0.001, 10])
...
% PRXSO2H
figure(9019)
loglog(SRXtot_vector, PRXSO2H_vector, 'Color', [51,102,255]/255, 'LineWidth', 3);
hold on  
% xlim([0.001, 10])
% xticks([0.001, 0.01, 0.1, 1, 10])
% ylim([0.001, 10])
...
%% --------------- Generate fig. S6C and D ---------------------------------------------------------------------- %%
y0 = cell2mat(struct2cell(default_init));
param = default_param;
param.k0 = 80; % run with different param.k0 values (80, 120, 160);
tspan0 = [0 : 1 : 3600*100]; 

tspan1 = [0 : 1 : 3600*50]; 
tspan2 = [3600*50 : 1 : 3600*250]; 
tspan3 = [3600*250 : 1 : 3600*300]; 
...
SRXtot_vector = [0.8, 0.4, 0.2, 0.15, 0.1, 0.05];
